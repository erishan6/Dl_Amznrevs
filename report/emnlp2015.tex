...
To briefly describe the purpose of activation functions is to add the non-linearity to the neural networks. Activation functions which are easily differentiable and easy to train should be used. So we trained the network for the two approaches described above using four different types of activation functions: [ReLU, Tanh, ELU and Softplus].
We used stardard implementations provided in Tensorflow with all the default parameters.
